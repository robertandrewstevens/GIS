...
> Sy5_nb <- graph2nb(soi.graph(Sy4_nb, coords), row.names = IDs) > Sy6_nb <- graph2nb(gabrielneigh(coords), row.names = IDs)
...
> set.seed(1234)
> boot2 <- boot(NY8$Cases, statistic = MoranI.pboot, R = 999, sim = "parametric", ran.gen = CR, listw = lw_B, n = length(NY8$Cases), S0 = Szero(lw_B), mle = rni)
...
Figure 9.13. Local Moran’s Ii values calculated directly and using the constant risk hypothesis
...
Figure 9.15. Probability values for census tracts in and near the city of Binghampton, local Moran’s Ii: normality assumption, exact values, and constant risk hypothesis
